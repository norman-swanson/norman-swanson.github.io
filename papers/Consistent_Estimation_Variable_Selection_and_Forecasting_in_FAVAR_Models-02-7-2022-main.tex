...
%TCIDATA{LastRevised=Monday, February 07, 2022 17:14:20}
...
http://econweb.umd.edu/\symbol{126}chao/Research/research\_files/.} The
technical supplement is organized into four subappendices. Appendix A
provides proofs of the main theorems. Appendix B provides proofs of lemmas
used in proving Theorem 1, and Appendix C contains proofs of supporting
lemmas, used primarily in the proofs of Theorems 2 and 3. Finally, Appendix
D contains the proofs of supporting lemmas used primarily in the proofs of
Theorems 4 and 5.
...
components of $Z_{t}$, so that $PZ_{t}$ can be partitioned as follows:%
\begin{equation*}
\mathcal{P}Z_{t}=\left( 
\begin{array}{c}
...
_{t}+\varepsilon _{\cdot t+1}^{Y},  \label{Y component FAVAR} \\
F_{t+1} &=&\mu _{F}+A_{FY}\underline{Y}_{t}+A_{FF}\underline{F}%
_{t}+\varepsilon _{\cdot t+1}^{F},  \label{F component FAVAR}
\end{eqnarray}%
where%
\begin{eqnarray}
...
Z_{it}y_{\ell ,t{\LARGE +}1}$, for $t=\tau _{1}+p,...,\tau -p$, have not
...
choice, note first that, by applying Lemma C-17 given in Appendix C below,
we obtain that:%
\begin{equation}
P\left( \left\vert S_{i,\ell ,T}\right\vert \geq z\right) \leq 2\left[
1-\Phi \left( z\right) \right] \left( 1+o\left( 1\right) \right) ,
\label{moderate dev result}
\end{equation}%
which holds for all $i$ and $\ell $, for all $T$ sufficiently large, and for
all $z$ such that:%
\begin{equation*}
...
\end{equation*}%
In view of expression (\ref{moderate dev result}), we can interpret moderate
deviation as providing an asymptotic approximation of the (two-sided) tail
behavior of the statistic, $S_{i,\ell ,T},$ based on the tails of the
standard normal distribution. Indeed, the inequality in (\ref{moderate dev
result}) can be easily made into an equality by imposing a mild additional
restriction on the range for $z$\footnote{%
For example, if we restrict $z$ to lie in the range:%
\begin{equation*}
0\leq z\leq c_{0}\min \left\{ T^{\left( 1-\alpha _{{\large 1}}\right) \frac{%
{\large 1}}{{\large 6}}}/L\left( T\right) ,T^{\frac{{\Large \alpha }_{%
{\large 2}}}{{\large 2}}}\right\} ,
\end{equation*}%
for some slowly varying function $L\left( T\right) ,$ such that $L\left(
T\right) \rightarrow \infty $, then, we can show that:%
\begin{equation*}
P\left( \left\vert S_{i,\ell ,T}\right\vert \geq z\right) =2\left[ 1-\Phi
\left( z\right) \right] \left( 1+o\left( 1\right) \right) \text{.}
\end{equation*}%
}. Now, suppose initially that we wish simply to control the probability of
a Type I error for testing the null hypothesis $H_{0}:\gamma _{i}=0$ (i.e.,
the $i^{th}$ variable does not load on the underlying factors) at some fixed
...
the one which we analyze here. In consequence, the conditions we will
specify on the setting of the tuning parameter, $\varphi ,$ will also be
quite different from what they recommend in their paper.
...
of a false positive, i.e. $P\left( i\in \widehat{H}^{c}|\iota \in H\right) $%
, and the probability of a false negative, i.e., $P\left( i\in \widehat{H}%
|\iota \in H^{c}\right) $, approach zero as $N$, $T\rightarrow \infty $. To
show this result, we must first state a number of additional assumptions.
...
of this matrix, where, in this case,%
\begin{equation*}
...
}}_{t}$\textit{\ be as defined as in expression (\ref{factor estimator}).
...
\begin{equation*}
\widehat{Y}_{T{\LARGE +}h}\mathit{-}\left( \beta _{0}+B_{1}^{\prime }%
\underline{Y}_{T}+B_{2}^{\prime }\underline{F}_{T}\right) \overset{p}{%
