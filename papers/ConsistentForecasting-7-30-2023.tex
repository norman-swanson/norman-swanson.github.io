...
%TCIDATA{LastRevised=Sunday, July 30, 2023 20:26:27}
...
we draw on results of Chao, Qiu, and Swanson (CQS: 2023a), where a
completely consistent variable selection procedure useful for specifying
FAVAR models is developed. We then establish that the conditional mean of
the infeasible $h$-step ahead forecasting equation implied by an FAVAR can
...
impetus for the work of Chao, Qiu, and Swanson (CQS: 2023a), where a
variable selection procedure is developed for pre-selecting relevant
predictor variables for use in the consistent estimation of latent factors
in an FAVAR model. Their variable selection procedure is based on the use of
easy to construct self normalized statistics measuring the covariation
between target variables to be predicted and possible predictor variables to
be used in factor estimation. CQS (2023a) show that for their procedure, the
...
estimated using predictor variables selected using the procedure of CQS
(2023a)\ are consistent, up to a rotation. With these results in hand, we
...
model, the variable selection procedure of CQS (2023a), and the assumptions
...
offers concluding remarks. Proofs of the main theorems and some supporting
lemmas are given in the appendices of this paper.
...
\noindent \qquad Following CQS (2023a), we begin by considering the
following $p^{th}$-order factor-augmented vector autoregression (FAVAR):%
\begin{equation}
...
the proofs of Lemmas C-4 and C5 in Appendix C below. It remains to define
the relationship between the $F_{t}$ and the variables used to extract these
factors. To do this, we assume that:%
\begin{equation}
...
Following Chao, Qiu, and Swanson (2023a), we assume that not all components
of $Z_{t}$ provide useful information for estimating $\underline{F}_{t}$,
...
result.\footnote{%
In statistics, there is a growing literature on the potential inconsistency
of sample eigenvectors in high dimensional problems, as discussed in Paul
(2007), Johnstone and Lu (2009), Shen Shen, Zhu, and Marron (2016), and
Johnstone and Paul (2018).} As discussed in CQS (2023a), there is an
important related paper by Bai and Ng (2021) that establishes factor
estimator consistency for cases where $N/(TN_{1})\rightarrow 0$. For cases
where $N/(TN_{1})\rightarrow c$, or $N/(TN_{1})\rightarrow \infty $, where $c
$ is a constant, their result does not hold. In this paper, we establish
that consistency can be achieved in our context even if $N/(TN_{1})%
\nrightarrow 0$, if one pre-screens variables using the self-normalized
statistics outlined below. This is important because the degree of factor
pervasiveness is ultimately data dependent, and one way to estimate $N_{1}$
invloves utilizing the variable screening statistic that is discussed in the
sequel.
...
discussion of this Assumption, see CQS (2023a).

Note that Assumptions 3-1, 3-2(a)-(c), and 3-7 are sufficient to prove Lemma
C-5 in Appendix C\footnote{%
For a statement and proof of Lemma C-5, see Appendix C below.}, which states
that the process $\left\{ W_{t}\right\} $ generated by the FAVAR model given
in expression (\ref{FAVAR}) is a $\beta $-mixing process with $\beta $%
...
outlined in CQS (2023a), and draws on pathbreaking moderate deviation
results from Chen, Shao, Wu, and Xu (2016). To accommodate data dependence,
consider self-nomalized statistics that are constructed from observations
which are first split into blocks in a manner similar to the kind of
construction one would employ in implementing a block bootstrap or in
proving a central limit theorem using the blocking technique. One such
statistic has the form of an $\ell _{\infty }$ norm and is given by: 
\begin{equation}
...
standard normal as the threshold function, refer to CQS (2023a), and note
...
refer to CQS (2023a). Given the above assumptions, Theorem 1 of CQS (2023a)
...
CQS (2023a) also note that the above variable selection procedure provides
us with a consistent estimate $\widehat{N}_{1}$ of the unobserved quantity $%
...
results reported in Section 3 of CQS (2023a) show that the variable
selection procedure discussed above performs very well in finite samples,
under the tuning parameter choice $\varphi =N^{-\frac{{\large 2}}{{\large 5}}%
}$, both in terms of controlling the probability of a false positive (or
Type I) error and in terms of controlling the probability of a false
...
\noindent \qquad If we examine the proof of Theorem 4.1 in Appendix 1 as
well as the supporting arguments given in the proof of Lemma D-15 in
Appendix D below, we see that two of the key components of the proof involve
showing that:%
\begin{equation*}
...
More specifically, in Appendix D below, we show that for $h$-step ahead
forecasts associated with the (infeasible) forecasting equation implied by
the FAVAR model (\ref{FAVAR}), we have the form%
\begin{equation}
...
statistic (see Chao, Qiu, and Swanson (CQS: 2023a) that correctly identifies
the set of variables which load significantly on the underlying factors,
with probability approaching one, as the sample sizes go to infinity.
Furthermore, CQS(2023a) show that estimating the factors using only those
variables selected by their method allows factors to be consistently
estimated, up to an invertible matrix transformation, even in certain
situations where the standard pervasiveness assumption does not hold,
provided that the number of relevant variables is sufficiently large. Using
the factors estimated in such a manner, we show that the conditional mean
function of a factor-augmented forecasting equation can be consistently
estimated, even for the case of multi-step ahead forecasts.

\section{\noindent Appendix A: Proofs of Theorems 2.1, 4.1, and 4.2\qquad}

\noindent \textbf{Proof of Theorem 2.1:}

The proof of Theorem 2.1 requires a long series of calculations. Hence, we
...
\section{\noindent Appendix C: Lemmas Used in the Proofs of the Key
Supporting Lemmas Given in Appendix D.}

\noindent \qquad Lemmas C-1, C-2, C-3, C-4, and C-5 correspond,
respectively, to Lemmas OA-1, OA-2, OA-3, OA-5, and OA-11 in Chao, Qiu, and
Swanson (2023b). However, since these lemmas are used to prove key
supporting lemmas given in Appendix D below, for readers' convenience, we
...
<\infty
\end{equation*}%
\textbf{Proof of Lemma C-1: }By the integral test,%
\begin{equation*}
\dsum\limits_{m=1}^{\infty }m^{G}\exp \left\{ -am^{{\large \theta }}\right\}
...
\end{equation*}%
\textbf{Remark: }This is Corollary 14.3 of Davidson (1994). For a proof, see
pages 212-213 of Davidson (1994).

\noindent \medskip

\noindent \textbf{Lemma C-4:} Suppose that Assumptions 3-1, 3-2(a)-(b), 3-5,
...
\right) \text{. }
\end{equation*}%
\textbf{Proof of Lemma C-4:}

To proceed, note that, given Assumption 3-1, we can write the vector
...
\end{eqnarray*}%
Now, by Assumption 3-7, there exists a constant $\underline{C}>0$ such that 
\begin{eqnarray*}
...
$ and where $0<\phi _{\max }<1$ since, by Assumption 3-1, all eigenvalues of 
$A$ have modulus less than $1$. It follows by Assumption 3-5 that, there
...
where $\phi _{\max }=\max \left\{ \left\vert \lambda _{\max }\left( A\right)
\right\vert ,\left\vert \lambda _{\min }\left( A\right) \right\vert \right\} 
$ and where $0<\phi _{\max }<1$ given that Assumption 3-1 implies that all
eigenvalues of $A$ have modulus less than $1$. Now, in light of Assumption
3-2(b), we can set $C\geq 1$ to be a constant such that $E\left\Vert
...
\noindent \textbf{Lemma C-5: }Let $W_{t}=\left( Y_{t}^{\prime
...
To prove this lemma, we shall verify the conditions of Lemma OA-8 of Chao,
Qiu, and Swanson (2023b) for the vector moving-average representation of $%
W_{t}$, i.e., 
\begin{equation*}
...
\end{equation}%
and note first that, setting $\delta =5$ in Lemma OA-8 of Chao, Qiu, and
Swanson (2023b), and we see that Assumptions (i) and (ii) of this lemma are
the same as the conditions specified in Assumption 3-2 (a)-(c). Next, note
that, since in this case $\Psi _{j}=J_{d{\LARGE +}K}A^{j}J_{d{\LARGE +}%
K}^{\prime }$, we have 
\begin{eqnarray*}
...
\right\} \right] \text{ \ }\left( \text{by Assumption 3-7}\right) \\
...
&=&\dsum\limits_{j=0}^{\infty }\left( \dsum\limits_{k=j}^{\infty }\left\Vert
\Psi _{k}\right\Vert _{2}\right) ^{\frac{{\large 5}}{{\large 6}}} \\
...
}{{\large 6}}}\right] ^{-2}\text{ }\left( \text{by Lemma OA-10 of Chao, Qiu,
and Swanson (2023b)}\right) \\
&<&\infty \text{ \ }\left( \text{since }0<\phi _{\max }^{\frac{{\large 5}}{%
{\large 6}}}<1\text{ given that }0<\phi _{\max }<1\right) \text{.}
\end{eqnarray*}

Hence, all conditions of Lemma OA-8 of Chao, Qiu, and Swanson (2023b) are
fulfilled. Applying this lemma, we then obtain that there exists a constant $%
\overline{C}$ such that%
\begin{eqnarray*}
...
(a) of Lemma C-2 to deduce that $\left\{ W_{t}\right\} $ is a $\beta $
...
\section{\noindent Appendix D: Key Supporting Lemmas Used in the Proofs of
...
Now, to show part (b), note first that expression (\ref{VMA representation
of W}) in the proof of Lemma C-4 in Appendix C above gives a vector
...
inequality and the result given in Lemma C-4.

Next, note that, by Lemma C-5, $\left\{ W_{t}\right\} $ is $\beta $-mixing
...
inequality and the existence of moment result given in Lemma C-4, it then
follows that%
\begin{eqnarray*}
\frac{1}{T_{h}^{2}}\dsum\limits_{t=p}^{T-h}E\left[ \left( a^{\prime }%
\underline{W}_{t}\right) ^{2}\left( \eta _{t+h}^{\prime }b\right) ^{2}\right]
...
\text{ \ }\left( \text{applying Lemma C-4 and expression (\ref{Eeta^4}) above%
}\right) \\
...
by Assumption 3-5 and Lemma C-4 that there exists a positive constant $C$
such that%
\begin{eqnarray*}
...
proof of part (a) of Theorem 1 (see Chao, Qiu, and and Swanson (2023b)), we
see that there exists a constant $C>2d$ such that%
\begin{eqnarray*}
...
Assumptions 3-3(b) and 3-5 and Lemma C-4; we get%
\begin{eqnarray*}
&&\frac{1}{N_{1}^{2}T_{h}^{2}}\dsum\limits_{i\in H^{{\large c}%
...
in the proof of part (a) of Theorem 1 (see Chao, Qiu, and Swanson (2023b)),
we see that, for any $i\in H$, there exists a constant $C_{2}>2d$ such that%
\begin{equation*}
...
&\leq &\overline{C}^{\frac{{\large 1}}{{\large 3}}}\text{ \ (by Lemma C-4)}
\label{bd on MFF} \\
&<&\infty  \notag
\end{eqnarray}%
from which it follows that%
...
\right\} ^{c}
\end{eqnarray*}%
Hence, applying either part (a) or part (b) of Theorem 2 in Chao, Qiu, and
Swanson (2023a) depending on whether $\mathbb{S}_{i,T}^{+}=\dsum\nolimits_{%
\ell =1}^{d}\varpi _{\ell }\left\vert S_{i,\ell ,T}\right\vert $ or $\mathbb{%
S}_{i,T}^{+}=\max_{1\leq \ell \leq d}\left\vert S_{i,\ell ,T}\right\vert $,
we obtain 
\begin{eqnarray*}
...
orthogonal matrix. In addition, further application of Lemma D-13 shows that 
...
\epsilon \right\}  &=&\left\{ \left\vert \frac{1}{N_{1}}\dsum%
\limits_{i=1}^{N}\mathbb{I}\left\{ i\in \widehat{H^{c}}\right\}
...
,T}\right\vert $, we can apply the results of Theorem 2 in Chao, Qiu, and
Swanson (2023a) to obtain%
\begin{eqnarray*}
...
Theorem 1 in }\right.  \\
&&\left. \text{Chao, Qiu, and Swanson (2023a)\footnote{%
The statement of this theorem can be found in Chao, Qiu, and Swanson
(2023a), but its proof is actually given in the online supplement - Chao,
Qiu, and Swanson (2023b).}}\right)  \\
...
\mathbb{I}\left\{ i\in \widehat{H^{c}}\right\} \gamma _{i}-\gamma
_{i}\right)  \\
...
\sqrt{N_{1}}}\right\Vert _{F}^{2}\geq \epsilon \right)  \\
&\leq &\frac{1}{\epsilon }E\left\{ \frac{1}{N_{1}}\dsum\limits_{i\in H^{%
{\large c}}}\gamma _{i}^{\prime }\gamma _{i}\left[ 1-\mathbb{I}\left\{ i\in 
\widehat{H^{c}}\right\} \right] \right\}  \\
...
by Theorem 2 in Chao, Qiu, and Swanson (2023a)}\right) 
\end{eqnarray*}%
from which we further deduce that%
\begin{equation*}
\left\Vert \frac{\Gamma \left( \widehat{H^{c}}\right) -\Gamma }{\sqrt{N_{1}}}%
...
&\rightarrow &0\text{ ,}
\end{eqnarray*}%
since, by Theorem 2 in Chao, Qiu, and Swanson (2023a), $P\left( \min_{i\in
...
the proof of Theorem 1 in Chao, Qiu, and Swanson (2023b),%
\begin{eqnarray*}
0 &\leq &E\left[ \frac{1}{N_{1}}\dsum\limits_{i\in H}\mathbb{I}\left\{ i\in 
\widehat{H^{c}}\right\} \right]  \\
&=&\frac{1}{N_{1}}\dsum\limits_{i\in H}\Pr \left( i\in \widehat{H^{c}}%
\right)  \\
...
where $\mathbb{S}_{i,T}^{+}=\max_{1\leq \ell \leq d}\left\vert S_{i,\ell
,T}\right\vert $, we have, by applying an argument similar to that given in
the proof of Theorem 1 as shown in Chao, Qiu, and Swanson (2023b),%
\begin{eqnarray*}
&&\Pr \left\{ \frac{1}{N_{1}}\dsum\limits_{i\in H}\mathbb{I}\left\{ i\in 
...
\end{equation}%
In addition, note that by applying Lemma C-4 and the Markov's inequality in
...
=1}^{d}\varpi _{\ell }\left\vert S_{i,\ell ,T}\right\vert $ or the case
where $\mathbb{S}_{i,T}^{+}=\max_{1\leq \ell \leq d}\left\vert S_{i,\ell
,T}\right\vert $, we have, by applying Theorem 2 in Chao, Qiu, and Swanson
(2023a),%
\begin{eqnarray*}
0 &\leq &E\left[ \frac{1}{N_{1}}\dsum\limits_{i\in H^{{\large c}}}\left( 1-%
\mathbb{I}\left\{ i\in \widehat{H^{c}}\right\} \right) \right]  \\
...
&&\left( \text{following an argument similar to that given in the proof of
Theorem 1 }\right.  \\
&&\left. \text{as shown in Chao, Qiu, and Swanson (2023b)}\right)  \\
&\rightarrow &0\text{ as }N_{1},N_{2},T\rightarrow \infty 
\end{eqnarray*}%
Moreover, making use of part (b) of Assumption 3-3, we have%
\begin{eqnarray*}
...
\frac{{\Large 1}}{{\Large 3}}}\text{ \ }\left( \text{by Lemma C-4}\right) 
\notag \\
...
\bibitem{} Chao, J. C., K. Qiu, and N. R. Swanson (2023a): \textquotedblleft
Selecting the Relevant Variables for Factor Estimation in Factor Estimation
in FAVAR Models, With An Application to Forecasting the Yield
Curve,\textquotedblright\ Working Paper, Rutgers University and University
of Maryland.

\bibitem{} Chao, J.C., K. Qiu, and N. R. Swanson (2023b): \textquotedblleft
Online Supplement: Selecting the Relevant Variables for Factor Estimation in
FAVAR\ Models, With An Application to Forecasting the Yield Curve," Working
Paper, Rutgers University and University of Maryland.
